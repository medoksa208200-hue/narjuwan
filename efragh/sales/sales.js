...
function render(){
 ...
 const tr=document.createElement('tr');for(const value of [unit.project_name,unit.building_no,(unit.asset_kind==='land'?'قطعة ':'وحدة ')+unit.unit_no,unit.total_area==null?'—':new Intl.NumberFormat('ar-SA').format(unit.total_area)+' م²']){const td=document.createElement('td');td.textContent=value||'—';tr.append(td);}
 const stateCell=document.createElement('td');const badge=document.createElement('span');badge.className='badge '+statusClass(statusOf(unit));badge.textContent=statusOf(unit);stateCell.append(badge);tr.append(stateCell);
 const action=document.createElement('td');const button=document.createElement('button');button.textContent='تغيير الحالة';button.addEventListener('click',()=>{selected=unit;$('unitLabel').textContent=unit.project_name+' · '+(unit.asset_kind==='land'?'بلوك ':'عمارة ')+unit.building_no+' · '+(unit.asset_kind==='land'?'قطعة ':'وحدة ')+unit.unit_no+' — الحالة الحالية: '+statusOf(unit);$('newStatus').value=statusOf(unit);$('editError').hidden=true;$('edit').showModal();});action.append(button);tr.append(action);$('rows').append(tr);}
